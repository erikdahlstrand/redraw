...
describe('Rectangle tool', () => {
    ...
    beforeEach(() => {
        ...
        new HorizontalLineTool({ canvas: {} }, eventAggregator, {}, fabric);
    });

    it('should register for tool activation event', () => {
        expect(subscribeToSpy.calledOnce).to.equal(true);
        expect(subscribeToSpy.args[0][0]).to.equal('horizontalLine');
        expect(subscribeToSpy.args[0][1]).to.equal('HorizontalLineTool');
